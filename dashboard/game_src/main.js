...
/**
 * `options` comes from the React wrapper, which got it from the server:
 *   { teamNumber, startIndex, onProgress }
 *
 * The game used to be a standalone page that always began at level 1. Here a
 * team can close the tab on level 4 and come back, so the starting level is
 * told to it rather than assumed.
 */
export function initMain(options = {}) {

const TEAM_NUMBER = options.teamNumber ?? null;
...
function updateCircuit() {
  ...
  if (!gameWon && won) {
    ...
    // ── Submit ────────────────────────────────────────────────────────────
    // SEND THE BOARD, NOT THE VERDICT.
    //
    // This used to post { voltage, targetVoltage } — two numbers the browser
    // computed, which the server then compared against each other. That is the
    // browser marking its own homework: a hand-written request carrying
    // {"voltage":0,"targetVoltage":0} scored the level without the game ever
    // being opened.
    //
    // Now it posts only where the player put their own tiles. The server holds
    // the level (grid, source voltage, fixed modifiers, x-blocks, target) and
    // re-runs the solver itself, so the strongest thing a forged request can
    // claim is an arrangement of real tiles that either completes the circuit
    // or does not.
    const pieces = [];
    for (let r = 0; r < board.rows; r++) {
      for (let c = 0; c < board.cols; c++) {
        const cell = board.grid[r][c];
        if (cell) pieces.push({ row: r, col: c, type: cell.type, rotation: cell.rotation });
      }
    }

    // No teamNumber in the body any more: the round runs inside the dashboard,
    // so the server reads the signed session cookie instead. Sending it would
    // be at best ignored and at worst read as something the client gets to
    // decide. `TEAM_NUMBER` is still used to label the board on screen.
    fetch('/api/circuit/submit', {
      method: 'POST',
