...
// The chrome around the board — status strip, banners, footer, gate screens.
import "./circuit.css";
...
export default function CircuitGame({
  ...
  return (
    <>
      {/* ── The round bar ────────────────────────────────────────────────
          Status on the left, Back/Finish on the right, in ONE fixed bar that
          sits above the game's full-screen layers.

          Upstream these were two separate blocks — a strip above the game and a
          footer below it — and both were painted under `#game-wrapper`, which
          is `position: fixed; inset: 0`. The footer being invisible meant the
          round had no way out and no way to bank itself; see the note in
          circuit.css. */}
