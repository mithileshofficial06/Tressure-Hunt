...
/**
 * The themed screen shown before the room opens.
 *
 * WHY THIS IS SHORT, AND WHY IT USED TO BE EIGHT SECONDS. Upstream this ran a
 * fixed 8s timer, which read as "the 3D scene is loading". It is not: this
 * component lives INSIDE the same lazy chunk as the room, so by the time a
 * single frame of it renders, three.js and all 7,000 lines of scene have
 * already arrived over the network. The bar was measuring nothing. Eight
 * seconds of nothing, then a click, before a team could start looking — five
 * times over on an event day.
 *
 * What is left to wait for after this mounts is the Canvas building its WebGL
 * context, which is on the order of a second. So the bar is short enough to
 * cover that and no longer, and SKIP is there for a team that has seen it four
 * times already.
 */
export function SpiderLoadingScreen({
  ...
  return (
    /**
     * ABSOLUTE, NOT FIXED — and it has to be.
     *
     * This was `fixed inset-0`, which upstream covered the whole window because
     * the room was the whole page. Here the round is a panel inside the
     * dashboard's column, and that column carries `.anim-rise`. That animation
     * ends on `transform: translateY(0)` with `fill-mode: both`, so the div
     * keeps a non-`none` transform forever — and a transformed ancestor becomes
     * the CONTAINING BLOCK for its `position: fixed` descendants. The overlay
     * was therefore being sized against that div rather than the viewport, and
     * came out as a squashed band with the title spilling out of the top.
     *
     * `absolute` against the room-sized wrapper in MysteryRoom.tsx is what was
     * meant all along: it fills exactly the box the room is about to occupy, so
     * there is also no layout jump when the two swap over.
     */
    <div className="absolute inset-0 z-50 flex flex-col items-center justify-center overflow-hidden bg-[#06040d] p-6">
      ...
      {/* SKIP. A team on its second visit — or one that has watched four other
          teams sit through this — should not have to. Deliberately quiet and
          out of the way, so it reads as an escape hatch rather than as the
          main action. */}
      {progress < 100 && (
        <button
          type="button"
          onClick={onComplete}
          className="absolute bottom-4 right-4 z-10 rounded border border-white/25 px-3 py-1.5 font-mono text-[0.65rem] tracking-widest text-white/60 transition-colors hover:border-white/60 hover:text-white"
        >
          SKIP →
        </button>
      )}
