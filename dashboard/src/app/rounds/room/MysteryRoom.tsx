...
/**
 * What the room reports upwards.
 *
 * Upstream this was `PuzzleProps` from SympoApp's `hunt/registry`, where every
 * puzzle is mounted by slug into a shared shell and so has to accept the same
 * three props. This dashboard has no registry — the room is an ordinary route —
 * so the contract is stated here instead of importing a registry that would
 * drag in four other puzzles.
 *
 * Both callbacks fire once, together, when the fifth section opens. `RoomRound`
 * is what listens; the room itself never talks to the server, exactly as
 * upstream.
 */
export interface MysteryRoomProps {
  /** The room's reveal code, once all five sections are open. */
  onSolve?: (code: string) => void;
  /** Same value, kept so the upstream call site below is unchanged. */
...
export default function MysteryRoom({ onAnswer, onSolve }: MysteryRoomProps) {
  ...
  /**
   * The intro sits in a box the same size as the room it is standing in for.
   *
   * Upstream returned the overlay bare, which worked only because it was
   * `fixed` and the room was the whole page. Both of those stopped being true
   * here: the round is a panel in a column, and that column is transformed by
   * `.anim-rise`, which silently makes it the containing block for anything
   * `fixed` inside it. Giving the overlay a real box to fill — the SAME box the
   * room uses below — fixes the positioning at the source and means the page
   * does not jump when the intro hands over to the scene.
   *
   * These dimensions must track the room wrapper's. If one changes, change both.
   */
  if (loading) {
    return (
      <div className="relative h-[78vh] min-h-[460px] w-full overflow-hidden bg-[#06040d]">
        <SpiderLoadingScreen onComplete={() => setLoading(false)} />
      </div>
    );
  }
